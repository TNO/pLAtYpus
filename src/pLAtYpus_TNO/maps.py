...
def get_long_term_averages(product, country, parameters):
    ...
    percentage_time_span_end_average = pLAtYpus_parameters[
...
def make_long_term_average_tables(parameters):
    ...
    pLAtYpus_parameters = parameters['pLAtYpus']
    stakeholders = pLAtYpus_parameters['stakeholders']
    long_term_averages_table_name_prefix = pLAtYpus_parameters[
        'long_term_averages_table_name_prefix'
    ]
    countries = parameters['survey']['countries']
    products = list(parameters['products'].keys())
    for product in products:
        long_term_averages_dataframe = pd.DataFrame(
            columns=stakeholders, index=countries
        )
        long_term_averages_dataframe.index.name = 'Country'
        for country in countries:
            long_term_averages = get_long_term_averages(
                product, country, parameters
            )
            for stakeholder in stakeholders:
                long_term_averages_dataframe.loc[country][
                    stakeholder
...
def make_product_area_map(product, parameters):
    ...
    country_code_header_in_map_data = maps_parameters[
    ...
    product_data[country_code_header] = product_data['Country'].map(
        country_code_dictionary
    )

    area_data = cook.get_map_area_data(parameters)

    product_plot = pd.merge(
        area_data,
        product_data,
    ...
    stakeholder_area_map_figure, (stakeholder_area_map_plots) = plt.subplots(
        1, len(stakeholders), figsize=(10 * golden, 10)
    )
    for stakeholder_index, stakeholder in enumerate(stakeholders):
        ...
        area_data.plot(
            ax=area_map_plot,
            facecolor=non_survey_country_color,
            edgecolor='face',
        )
        area_data.plot(
        ...
        color_axis.set_ylabel(f'Engagement level')

        if show_legend:
            stakeholders_color_axis = stakeholder_area_map_figure.get_axes()[
                -1
            ]
            ...
            stakeholders_color_axis.set_xlabel(
                f'Engagement level ', fontsize=24
            )
            stakeholder_area_map_plots[stakeholder_index].text(
                0,
                0,
                f'© EuroGeographics for the administrative boundaries',
                fontsize=6,
            )
        area_map_plot.text(
            0,
        ...
        new_line = '\n'
        plot_title = (
            f'Long-term average engagement of' f' {stakeholder} for {product}'
        )
        ...
        cook.save_figure(
            area_map_figure, plot_title, output_folder, parameters
        )
    ...
    stakeholder_area_map_figure.suptitle(
        f'Long-term average engagement for {product_display}', fontsize=32
    )
...
def make_relationships_maps(parameters):
    ...
    non_survey_country_color = maps_parameters['non_survey_country_color']
    map_title_font_size = maps_parameters['map_title_font_size']
    product_deviations_table = parameters['survey']['relation_definitions'][
    ...
    product_relations_score[country_code_header] = product_relations_score[
        'Country'
    ...
    relations_score_plot = pd.merge(
        area_data,
        product_relations_score,
        left_on=country_code_header_in_map_data,
        right_on=country_code_header,
    )

    relations_score_plot = relations_score_plot.set_index('Product')
    values_to_plot = relations_score_plot['Relation score'].values
    lowest_value_to_plot = values_to_plot.min().min()
    highest_value_to_plot = values_to_plot.max().max()
    display_reference_scale = cook.reference_scale(
        [lowest_value_to_plot, highest_value_to_plot], 1
    ...
    relations_score_map_figure, (relations_score_map_plots) = plt.subplots(
        1, len(products), figsize=(10 * golden, 10)
    )
    for product_index, product in enumerate(products):
        ...
        color_axis.set_ylabel(f'Relation score')
        if show_legend:
            products_color_axis = relations_score_map_figure.get_axes()[-1]

            products_color_axis.set_xticks(color_axis_yticks)
            products_color_axis.set_xticklabels(
                color_axis_ytick_labels, fontsize=24
            )
            products_color_axis.set_xlabel(f'Relation score', fontsize=24)
        ...
        new_line = '\n'
        plot_title = f'Relation score for {product}'
        product_display = product.replace('_', ' ')
        display_plot_title = f'Relation score' f'\n' f' for {product_display}'
        product_plot.set_title(
            display_plot_title, fontsize=map_title_font_size
        )
        product_figure.set_tight_layout(True)
        cook.save_figure(product_figure, plot_title, output_folder, parameters)
    ...
    relations_score_map_figure.suptitle(f'Relation scores', fontsize=32)

    relations_score_map_figure.savefig(
        f'{output_folder}/Relations scores.png', bbox_inches='tight'
    )
    relations_score_map_figure.savefig(
        f'{output_folder}/Relations scores.svg', bbox_inches='tight'
    )
...
def make_relationships_overlap_maps(parameters):
    ...
    non_survey_country_color = maps_parameters['non_survey_country_color']
    map_title_font_size = maps_parameters['map_title_font_size']
    product_overlap_table = parameters['survey']['relation_definitions'][
    ...
    product_overlap_score[country_code_header] = product_overlap_score[
        'Country'
    ].map(country_code_dictionary)
    ...
    values_header = parameters['survey']['relation_definitions'][
        'overlap_columns'
    ][0]
    values_to_plot = overlap_score_plot[values_header].values
    lowest_value_to_plot = values_to_plot.min().min()
    highest_value_to_plot = values_to_plot.max().max()
    display_reference_scale = cook.reference_scale(
        [lowest_value_to_plot, highest_value_to_plot], 1
    ...
    overlap_score_map_figure, (overlap_score_map_plots) = plt.subplots(
        1, len(products), figsize=(10 * golden, 10)
    )
    for product_index, product in enumerate(products):
        product_figure, product_plot = plt.subplots(1, 1)
        ...
        color_axis.set_ylabel(f'Relation overlap')
        if show_legend:
            products_color_axis = overlap_score_map_figure.get_axes()[-1]

            products_color_axis.set_xticks(color_axis_yticks)
            products_color_axis.set_xticklabels(
                color_axis_ytick_labels, fontsize=24
            )
            products_color_axis.set_xlabel(f'Relation overlap', fontsize=24)
        ...
        overlap_score_map_plots[product_index].set_title(product, fontsize=24)
        new_line = '\n'
        plot_title = f'Relation overlap for {product}'
        product_display = product.replace('_', ' ')
        display_plot_title = (
            f'Relation overlap' f'\n' f' for {product_display}'
        )
        product_plot.set_title(
            display_plot_title, fontsize=map_title_font_size
        )
    ...
    overlap_score_map_figure.suptitle(f'Relation overlap', fontsize=32)

    overlap_score_map_figure.savefig(
        f'{output_folder}/Relations overlap.png', bbox_inches='tight'
    )
    overlap_score_map_figure.savefig(
        f'{output_folder}/Relations overlap.svg', bbox_inches='tight'
    )
...
def make_area_maps(parameters):
    ...
    for product in products:
        ...
        print(
            product, (datetime.datetime.now() - product_timer).total_seconds()
        )
