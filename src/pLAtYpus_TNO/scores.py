...
def attention_score(stakeholder, time, yes, model_coefficients):
    ...
    attention_score = {
        action: attention_inputs[action][stakeholders.index(stakeholder)]
...
def enable_score(stakeholder, time, yes, model_coefficients):
    if stakeholder == 'citizens':
        ...
        survey_enable_parameters = model_coefficients[
            'survey_enable_parameters'
        ]

        enable_score_adopt = np.prod(
            [
                survey_scores.loc[f'{survey_enable_parameter}']['Adopt']
                for survey_enable_parameter in survey_enable_parameters
            ]
        )

        availability_threshold_adopt = model_coefficients[
        ...
        availability_threshold_leave = model_coefficients[
...
def category_intention_score(
    ...
    category_intention_score = {
        action: (np.array(component_scores[action]) * survey_component_weights)
        for action in survey_scores_actions
    }
    ...
    yes_codes_adopt = [
        ...
        for yes_function, yes_stakeholder in zip(
            yes_functions_adopt, yes_stakeholders
        )
    ]
    yes_codes_leave = [
        f'{yes_function}_{yes_stakeholder}'
        for yes_function, yes_stakeholder in zip(
            yes_functions_leave, yes_stakeholders
        )
    ]
    category_yes_codes = {'Adopt': yes_codes_adopt, 'Leave': yes_codes_leave}

    return category_intention_score, category_yes_codes


def get_categories_intention_scores(
    stakeholders, survey_scores, survey_scores_actions, intention_categories
):
...
def intention_score(stakeholder, time, yes, model_coefficients):
    intention_categories = model_coefficients['intention_categories']
    survey_scores_actions = model_coefficients['survey_scores_actions']
    stakeholders = model_coefficients['stakeholders']
    category_weights = model_coefficients['category_weights']
    categories_intention_scores = model_coefficients[
        'categories_intention_scores'
    ]
    categories_yes_codes = model_coefficients['categories_yes_codes']

    intention_score = {}
    for action in survey_scores_actions:
        action_intention_score = 0
        for category, weight in zip(
            intention_categories[stakeholder], category_weights[stakeholder]
        ):
